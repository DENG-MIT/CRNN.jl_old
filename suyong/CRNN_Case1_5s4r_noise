...
nr = 4
# initial_species = [1,2]
file_name = "Alr001_Alr0001_n5_i200_noise"
is_restart = false
...
for (i,u) in enumerate(u0_list)
    ...
    y = Array(solve(prob_trueode, alg, saveat = tsteps))
    y_noise = y + 0.05*rand(eltype(y), size(y))
    push!(ode_data, y_noise)
end
...
cb = function (p, l, pred; doplot = false, save_file = true)
    ...
    if save_file && rem(iter,100) == 0
        for (i,maxiter) in enumerate(maxiters_list)
            if i == 1
