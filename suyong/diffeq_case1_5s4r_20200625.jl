...
cb = function (p, l, pred; doplot = false, save_file = true)
    ...
    if save_file
        savefig(plt,"Results_loss_y_t_lr0001_multi_initials_2.png")
    end
...
"
opt = GradientDescent()
p_temp = DiffEqFlux.sciml_train(loss_neuralode, p0, opt, u0_list_train, cb = cb, maxiters = n_iter).minimizer
push!(p_iter, p_temp)
"
...
opt = Optim.KrylovTrustRegion()
p_temp = DiffEqFlux.sciml_train(loss_neuralode, p0, opt, u0_list_train, cb = cb, maxiters = n_iter).minimizer
push!(p_iter, p_temp)

save("CRNN_lr0001_multi_initials_2.jld", "p_iter", p_iter)
# CRNN = load("CRNN.jld")
